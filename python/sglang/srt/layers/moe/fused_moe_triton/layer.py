...
class FusedMoE(torch.nn.Module):
    ...
    def __init__(
        ...
        if _use_aiter and self.expert_map_cpu is not None and not global_server_args_dict["disable_shared_experts_fusion"]:
            expert_mask = torch.ones((self.num_experts + 1,), dtype=torch.int32)
            expert_mask[:-1] = ((self.expert_map_cpu >= 0) & (self.expert_map_cpu < self.num_experts)).to(torch.int32)
            expert_mask[-1] = 0
    ...
    def determine_expert_map(self) -> Tuple[int, Optional[torch.Tensor]]:
        ...
        Returns:
            Tuple[int, Optional[torch.Tensor]]: A tuple containing:
                - local_num_experts (int): The number of experts assigned
                    to the current rank, including fused shared experts
        ...
        num_local_shared_experts = self.num_fused_shared_experts
